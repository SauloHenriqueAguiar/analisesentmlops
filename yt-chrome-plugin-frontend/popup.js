...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const API_URL = 'http://172.23.36.235:5000/';
  ...
  chrome.tabs.query({ active: true, currentWindow: true }, async (tabs) => {
    ...
    if (match && match[1]) {
      ...
      outputDiv.innerHTML = `<div class="section-title">YouTube Video ID</div><p>${videoId}</p><p>Buscando comentários...</p>`;

      const comments = await fetchComments(videoId);
      if (comments.length === 0) {
        outputDiv.innerHTML += "<p>Nenhum comentário encontrado para este vídeo.</p>";
        return;
      }

      outputDiv.innerHTML += `<p>Buscada ${comments.length} comentarios. Realizando análise de sentimentos...</p>`;
      ...
      if (predictions) {
        ...
        outputDiv.innerHTML += `
          <div class="section">
            <div class="section-title">Resumo da Análise de Comentários</div>
            <div class="metrics-container">
              <div class="metric">
                <div class="metric-title">Total comentarios</div>
                <div class="metric-value">${totalComments}</div>
              </div>
              <div class="metric">
                <div class="metric-title">comentarios unicos</div>
                <div class="metric-value">${uniqueCommenters}</div>
              </div>
              <div class="metric">
                <div class="metric-title">Comprimento médio do comentário</div>
                <div class="metric-value">${avgWordLength} words</div>
              </div>
              <div class="metric">
                <div class="metric-title">Pontuação média de sentimento</div>
        ...
        outputDiv.innerHTML += `
          <div class="section">
            <div class="section-title">Resultados da análise de sentimentos</div>
            <p>Veja o gráfico de pizza abaixo para a distribuição de sentimentos.</p>
        ...
        outputDiv.innerHTML += `
          <div class="section">
            <div class="section-title">Tendência de sentimento ao longo do tempo</div>
        ...
        outputDiv.innerHTML += `
          <div class="section">
            <div class="section-title">Nuvem de palavras de comentários</div>
        ...
        outputDiv.innerHTML += `
          <div class="section">
            <div class="section-title">Os 25 principais comentários com sentimentos</div>
            <ul class="comment-list">
              ${predictions.slice(0, 25).map((item, index) => `
                <li class="comment-item">
                  <span>${index + 1}. ${item.comment}</span><br>
                  <span class="comment-sentiment">Sentimento: ${item.sentiment}</span>
                </li>`).join('')}
            </ul>
          </div>`;
      }
    } else {
      outputDiv.innerHTML = "<p>Este não é um URL válido do YouTube.</p>";
    }
  });
  ...
  async function fetchComments(videoId) {
    ...
    try {
      ...
      outputDiv.innerHTML += "<p>Erro ao buscar comentários</p>";
    }
    return comments;
  }
  ...
  async function getSentimentPredictions(comments) {
    try {
      ...
      if (response.ok) {
        ...
        throw new Error(result.error || 'Erro ao buscar previsões');
      }
    } catch (error) {
      console.error("Erro ao buscar previsões:", error);
      outputDiv.innerHTML += "<p>Erro ao buscar previsões.</p>";
      return null;
    }
  }
